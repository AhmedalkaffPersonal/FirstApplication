...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adapter =
            ArrayAdapter<String>(this@MainActivity, android.R.layout.simple_list_item_1, users)


        mDb.collection(USER_COLLECTION).whereEqualTo(EMAIL, auth.currentUser?.email).get()
        ...
        mBinding.apply {
            ...
            listviewUsers.setOnItemClickListener(object : AdapterView.OnItemClickListener {
                override fun onItemClick(
                    ...
                    mDatabaseReference.child(USER).child(userId).child(COUNTER).get()
                        .addOnSuccessListener {
                            try {
                                counter = if (it.value == null) 0 else it.value.toString().toInt()
                                Log.d(TAG, "Counter : ${counter}")
                                mDatabaseReference.child(USER).child(userId).child(COUNTER)
                                    .setValue(++counter)
                            } catch (e: Exception) {
